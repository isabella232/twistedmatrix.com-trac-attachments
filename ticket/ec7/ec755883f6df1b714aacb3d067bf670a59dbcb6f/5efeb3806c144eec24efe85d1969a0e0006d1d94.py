...
def cbCommand1 ():
    print "You are a command-1 donkey!"

def cbCommand2 ():
    print "You are a command-2 jackass!"

def cbCommand3 ():
    print "You are a command-3 tortoise!"
...
class EchoProtocol(protocol.Protocol):
    ...
    textBuffer = []
    shellCommands = {'cmd1' : cbCommand1, 'cmd2' : cbCommand2, 'cmd3' : cbCommand3}
        
    def crunchBuffer (self):
        tempReturn = ''
        for i in self.textBuffer:
    ...
    def parseData (self, data):
        if data == '\r\n':
            theString = self.crunchBuffer()
            if theString in self.shellCommands.keys():
                self.shellCommands[theString]()
        else:
