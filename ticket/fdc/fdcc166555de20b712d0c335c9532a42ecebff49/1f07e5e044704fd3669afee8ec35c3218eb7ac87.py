...
if psycopg2:
    try:
        ...
        psycopg2.extensions.POLL_OK = None

from twisted.enterprise import pgadbapi
...
_skip = None
if psycopg2 is None:
    _skip = "psycopg2 not installed"
elif psycopg2.extensions.POLL_OK is None:
    _skip = ("psycopg2 does not have async support "
             "(what? you're not running git master?)")
...
class FakeWrapper(pgadbapi._PollingMixin):
    """
    A mock subclass of L{pgadbapi._PollingMixin}.
    """
    reactor = FakeReactor()
    prefix = "fake-wrapper"
...
class PGADBAPIPollingMixingTestCase(Psycopg2TestCase):

    def test_empty(self):
        """
        The default L{pgadbapi._PollingMixin} implementation raises an
        exception on pollable().
        """
        p = pgadbapi._PollingMixin()
    ...
    def test_polling(self):
        """
        L{pgadbapi._PollingMixin} adds and removes itself from the reactor
    ...
    def test_interface(self):
        """
        L{pgadbapi._PollingMixin} correctly provides the
    ...
    def test_errors(self):
        """
        Unexpected results from poll() make L{pgadbapi._PollingMixin} raise an
        exception.
        """
        p = FakeWrapper()
        p._pollable = PollableThing()
        ...
        d = p.poll()
        return self.assertFailure(d, pgadbapi.UnexpectedPollResult)


class PGADBAPIConnectionTestCase(Psycopg2TestCase):

    def test_simpleConnection(self):
        """
        Just connecting and disconnecting works.
        """
        conn = pgadbapi.Connection()
    ...
    def test_multipleConnections(self):
        ...
        d.addCallback(lambda c: conn.connect(
                ...
                host=DB_HOST, database=DB_NAME))
        d = self.failUnlessFailure(d, pgadbapi.AlreadyConnected)
...
class PGADBAPIManualQueryTestCase(_SimpleDBSetupMixin, Psycopg2TestCase):
...
class NotRollingBackCursor(pgadbapi.Cursor):
    """
    ...
    def _doit(self, name, *args, **kwargs):
        ...
        return pgadbapi.Cursor._doit(self, name, *args, **kwargs)


class PGADBAPIQueryTestCase(_SimpleDBSetupMixin, Psycopg2TestCase):
    ...
    def test_errorOnRollback(self):
        ...
        L{pgadbapi.RollbackFailed} failure that has references to the faulty
        ...
        def check_error(f):
            f.trap(pgadbapi.RollbackFailed)
...
class PGADBAPIConnectionPoolTestCase(Psycopg2TestCase):

    def setUp(self):
        self.pool = pgadbapi.ConnectionPool(
...
class PGADBAPIConnectionPoolHotswappingTestCase(Psycopg2TestCase):
    ...
    def test_removeWhileBusy(self):
        ...
        an exception.
        """
        pool = pgadbapi.ConnectionPool(None, database="psycopg2_test", min=1)
        d = pool.start()
