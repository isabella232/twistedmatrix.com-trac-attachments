...
class BaseExec(protocol.Protocol):
    def __init__(self, argv):
        self._argv = argv
        self._stdout_stream = None
        self._stdin_stream = None
        self._stderr_stream = None
        self._stdio_thread = None
        self._streams_ready = Event()
        
    def connectionMade(self):
        print("connectionMade")
        self._streams_ready.wait()
        self._stdio_thread = threading.Thread(target=self._standard_output_loop)
        self._stdio_thread.daemon = True
        self._stdio_thread.start()

    def dataReceived(self, data):
        self._stdin_stream.write(data)

    def _standard_output_loop(self):
        print("_standard_output_loop")
        read_fds = [self._stdout_stream.fileno()]

        while read_fds:
            #pdb.set_trace()
            ready_fds, _, _ = select(read_fds, [], [])
            if self._stdout_stream.fileno() in ready_fds:
                ret = os.read(self._stdout_stream.fileno(), 1024) 
                if not ret:
                    read_fds.remove(self._stdout_stream.fileno())
                    continue
                self.transport.write(ret)

        self.transport.connectionLost(reason="remote process exited")

class ExecCommand(BaseExec):
    def __init__(self, command):
        BaseExec.__init__(self, ["/bin/bash", "-c", command])
        ppp = subprocess.Popen(["/bin/bash", "-c", command], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
        self._stdout_stream, self._stdin_stream, self._stderr_stream = ppp.stdout, ppp.stdin, ppp.stderr
...
class ExampleSession(object):
    ...
    def execCommand(self, proto, cmd):
        ...
        """
        print("execCommand")
        prot = ExecCommand(cmd)
        prot.makeConnection(proto)
        proto.makeConnection(session.wrapProtocol(prot))
