...
def sendmail(
    ...
    """
    # Create a context factory with a particular SSL method which verifies the
    # peer's certificate.
    contextFactory = CertificateOptions(method=SSLv3_METHOD) # , verify=True)
...
class SendmailOptions(Options):
    ...
    optParameters = [
        ('username', 'u', None, 'The username with which to authenticate to the SMTP server.'),
        ('password', 'p', None, 'The password with which to authenticate to the SMTP server.'),
